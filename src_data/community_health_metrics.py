...
def main():
    ...
    languagecode = wikilanguagecodes[0]
...
def participation_metrics(languagecode):
    ...
    for dump_path in d_paths:
        ...
        while line != '':
            ...
            # MONTHLY AVERAGE SECONDS BETWEEN EDITS
            if seconds_since_last_edit != None and seconds_since_last_edit != '':
                if event_user_id != '' and event_user_id != 0:
                    try:
            ...
            if event_user_first_edit_timestamp != '' and event_user_id not in survived_dict:
                ...
                if event_timestamp_dt >= first_edit_timestamp_1day_dt and event_timestamp_dt <= first_edit_timestamp_7days_dt:
                    ...
                    if event_user_id in editor_user_page_edits:
                        survival_measures.append((event_user_id, event_user_text, editor_user_page_edits[event_user_id], None, 'user_page_edit_count_24h', first_edit_timestamp_1day_dt.strftime('%Y-%m'),first_edit_timestamp_1day_dt.strftime('%Y-%m-%d %H:%M:%S')))

                    if event_user_id in editor_user_page_talk_page_edits:
                        survival_measures.append((event_user_id, event_user_text, editor_user_page_talk_page_edits[event_user_id], None, 'user_page_talk_page_edit_count_24h', first_edit_timestamp_1day_dt.strftime('%Y-%m'),first_edit_timestamp_1day_dt.strftime('%Y-%m-%d %H:%M:%S')))
                ...
                if event_timestamp_dt >= first_edit_timestamp_1months_dt and event_timestamp_dt <= first_edit_timestamp_2months_dt:
                    ...
                    if event_user_id in editor_user_page_edits:
                        survival_measures.append((event_user_id, event_user_text, editor_user_page_edits[event_user_id], None, 'user_page_edit_count_1month', first_edit_timestamp_1day_dt.strftime('%Y-%m'),first_edit_timestamp_1day_dt.strftime('%Y-%m-%d %H:%M:%S')))

                    if event_user_id in editor_user_page_talk_page_edits:
                        survival_measures.append((event_user_id, event_user_text, editor_user_page_talk_page_edits[event_user_id], None, 'user_page_talk_page_edit_count_1month', first_edit_timestamp_1day_dt.strftime('%Y-%m'),first_edit_timestamp_1day_dt.strftime('%Y-%m-%d %H:%M:%S')))
    ...
    query = 'INSERT OR IGNORE INTO '+languagecode+'wiki_editor_metrics (user_id, user_name, value, metric_name, year_month, timestamp) VALUES (?,?,?,?,?,?);'
    ...
    for user_id, user_name in user_id_user_name_dict.items():
        ...
        try:
            se = editor_seconds_since_last_edit[user_id]
        except:
            se = ''



        user_characteristics.append((user_id, user_name, bot, user_flag, registration_date, year_month_registration,  fe, year_month, survived60d, le, lifetime_days, days_since_last_edit, se))


        # if user_name == 'Bff':
        #     print ((user_id, user_name, bot, user_flag, registration_date, year_month_registration, fe, year_month, survived60d, le, lifetime_days, days_since_last_edit))
        #     print (event_timestamp_dt)
        #     input('')


    query = 'INSERT OR IGNORE INTO '+languagecode+'wiki_editors (user_id, user_name, bot, user_flag, registration_date, year_month_registration, first_edit_timestamp, year_month_first_edit, survived60d, last_edit_timestamp, lifetime_days, days_since_last_edit, seconds_between_last_two_edits) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?);'
    ...
    for event_user_id, edit_count in user_id_edit_count.items():
        edit_counts.append(((event_user_id, user_id_user_name_dict[event_user_id], edit_count, None, 'edit_count', lym, '')))
...
def time_based_metrics(languagecode):
    ...
    for row in cursor.execute(query):
        ...
        if cur_user_id != old_user_id and old_user_id != '':
            ...
            if months_since_last_edit > 0:
                ...
                months_since_last_edit_dict[old_user_id] = months_since_last_edit
            ...
            if months_since_last_edit > max_inactive_months_row:
                ...
                parameters.append((old_user_id, old_user_name, 1, 'drop_off_threshold', cycle_year_month,''))
            else:
            ...
            max_inactive_months_row_dict[old_user_id] = max_inactive_months_row

            # print ('****')
            # print (old_user_id, old_user_name, active_months, max_active_months_row, inactive_months, max_inactive_months_row, total_months, months_since_last_edit)
    ...
    if months_since_last_edit > max_inactive_months_row:
        parameters.append((old_user_id, old_user_name, months_since_last_edit, None, 'max_inactive_months_row', old_year_month,''))
        parameters.append((old_user_id, old_user_name, 1, 'over_personal_drop_off_threshold', cycle_year_month,''))

    else:
        parameters.append((old_user_id, old_user_name, max_inactive_months_row, None, 'max_inactive_months_row', old_year_month,''))                
    parameters.append((old_user_id, old_user_name, inactivity_periods, 'inactivity_periods', old_year_month,''))
    ...
    bins = [0, 100, 500, 1000, 5000, 10000, 50000, 100000, 500000, 1000000] # in my phd I used, [1, 100, 1000, 5000, 10000, +)

    df['edit_count_bin'] = pd.cut(df['edits'], bins) 
    df['drop_off_bin_threshold'] = df['max_inactive_months_row'].groupby(df['edit_count_bin']).transform('mean')
    df['over_edit_count_bin_drop_off_threshold'] = df['drop_off_bin_threshold'] - df['months_since_last_edit']
    ...
    for index, rows in df.iterrows():
        ...
        over_edit_count_bin_drop_off_threshold = rows['over_edit_count_bin_drop_off_threshold']
        edit_count_bin = str(rows['edit_count_bin']).replace(' ','_').replace(',','')[1:]
        edit_count_bin = edit_count_bin[:len(edit_count_bin)-2]

        if over_edit_count_bin_drop_off_threshold < 0:
            parameters.append((user_id, user_name, 1, 'over_edit_count_bin_drop_off_threshold', cycle_year_month,''))
    ...
    df = df.loc[(df["over_edit_count_bin_drop_off_threshold"] < 0)]
    editors_bin_count = df.groupby(df['edit_count_bin']).size().to_dict()
    for bin, count in editors_bin_count.items():
        bin = bin.replace(' ','_').replace(',','')
        bin = bin[1:][:len(bin)-2]
...
def community_metrics(languagecode):
    ...
    year_months = set()
    for v in values:
        query = 'SELECT count(distinct user_id), year_month FROM '+languagecode+'wiki_editor_metrics WHERE metric_name = "monthly_edits" AND value >= '+str(v)+' GROUP BY year_month ORDER BY year_month'
        for row in cursor.execute(query):
            # print (row)
            value=row[0];
            year_month=row[1]
    ...
    queries_retention_dict['editors_edited_24h_7d_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "edit_count_24h" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'

    # number of editors who edited at least once between 7 days and 30 days after the first edit
    queries_retention_dict['editors_edited_7d_30d_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "edit_count_7d" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'

    # number of editors who edited at least once between 30 days and 60 days after the first edit
    queries_retention_dict['editors_edited_30d_60d_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "edit_count_30d" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'

    # number of editors who edited at least once 60 days after the first edit
    queries_retention_dict['editors_edited_60d_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "edit_count_60d" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'
    ...
    queries_retention_dict['editors_edited_user_page_24h_7d_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "user_page_edit_count_24h" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'

    # number of editors who edited their user_page at least once between 30 days and 60 days after their first edit
    queries_retention_dict['editors_edited_user_page_30d_60d_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "user_page_edit_count_1month" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'

    # number of editors who edited their user_page at least once
    queries_retention_dict['editors_edited_user_page_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "edit_count_ns2_user" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'

    # number of editors who edited their user_page_talk_page at least once between 24h and 7 days after their first edit
    queries_retention_dict['editors_edited_user_page_talk_page_24h_7d_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "user_page_talk_page_edit_count_24h" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'

    # number of editors who edited their user_page_talk_page at least once between 30 days and 60 days after their first edit
    queries_retention_dict['editors_edited_user_page_talk_page_30d_60d_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "user_page_talk_page_edit_count_1month" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'

    # number of editors who edited their user_page_talk_page at least once after the first edit
    queries_retention_dict['editors_edited_user_page_talk_page_afe'] = 'SELECT count(distinct ch.user_id), ch.year_month_first_edit FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "edit_count_ns3_user_talk" AND ce.value > 0 GROUP BY 2 ORDER BY 2 ASC;'
    ...
    for year_month in ym:
        query = 'SELECT ce.value FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "monthly_edits" AND year_month="'+year_month+'" AND ch.bot = "editor" AND ce.value > 0;'

        query = 'SELECT value FROM '+languagecode+'wiki_editor_metrics WHERE metric_name = "monthly_edits" AND year_month="'+year_month+'"'
        values = []
        for row in cursor.execute(query): values.append(row[0]);
        v = gini(values)
        # print ((v, 'gini_monthly', year_month))
        parameters.append((v, None, 'gini_monthly_edits', None, year_month))


    # query = 'SELECT ce.value FROM '+languagecode+'wiki_editors ch INNER JOIN '+languagecode+'wiki_editor_metrics ce ON ch.user_id = ce.user_id WHERE ce.metric_name = "edit_count" AND ch.bot = "editor" AND ce.value > 0;'
    # values = []
    # for row in cursor.execute(query): values.append(row[0]);
    # v = gini(values)
    # print (v)
...
if __name__ == '__main__':
    ...
    wikilanguages_utils.finish_email(startTime,'community_health_metrics.out', 'WIKIPEDIA DIVERSITY OBSERVATORY')
