...
content_type_dict = {'Regions':'regions','Subregions':'subregions','Countries':'countries','Gender':'gender','Top CCC Articles':'top_ccc_articles_lists', 'Languages CCC':'ccc'}
...
periods = []; entities_list = []



# CREATED DATA ################################################################################
query = 'select set1, set1descriptor, set2, set2descriptor, abs_value, rel_value, period from wcdo_intersections_monthly where content="articles" and set1descriptor NOT IN ("top10pageviews","top100pageviews","top500pageviews","top1000pageviews") order by set1, period;'
df_created = pd.read_sql_query(query, conn)

# #df_created.to_csv('mec.csv')
# df_created = pd.read_csv('mec.csv')
periods = sorted(list(df_created.period.unique()),reverse=True)
default_period = periods[0]#'2019-07'
df_created['period'] = pd.to_datetime(df_created['period'])
df_created = df_created.rename(columns={'set1':'Wiki','set1descriptor':'Group','abs_value':'Articles','rel_value':'Extent Articles (%)'}).round(1)
df_created['Language'] = df_created['Wiki'].map(language_names_full)
df_created['Region']=df_created['Wiki'].map(languages.region)

# wp
df_created_wp = df_created.loc[(df_created['set2descriptor'].isin(["ccc","not_own_ccc"])) & (df_created['Group']=='wp') & (df_created['Wiki']==df_created['set2'])]
df_created_wp = df_created_wp.rename(columns={'set2descriptor':'Content_Type'})
index_list = []
units_val = 0
old_period = 0
index_to_delete = []
for i, row in df_created_wp.iterrows():
    cur_period = row['period']
    cur_index = i
    if cur_period != old_period and old_period != 0:
        index_list.remove(old_index)
        index_to_delete += index_list
        index_list = []
        df_created_wp.at[old_index, 'Articles'] = units_val
        df_created_wp.at[old_index, 'Extent Articles (%)'] = '100.0'
        df_created_wp.at[old_index, 'Content_Type'] = 'wp'
        units_val = 0
    old_period = cur_period
    index_list.append(cur_index)
    units_val += row['Articles']
    old_index = cur_index
df_created_wp.at[old_index, 'Articles'] = units_val
df_created_wp.at[old_index, 'Content_Type'] = 'wp'
df_created_wp = df_created_wp.drop(index_to_delete)
df_created_wp['Entity'] = 'Entire Wikipedia'

# language CCC
df_created_ccc = df_created.loc[(df_created['set2descriptor'] == "ccc") & (df_created['Group']=='wp')]
df_created_ccc = df_created_ccc.rename(columns={'set2':'Entity_Extra','set2descriptor':'Content_Type'})
df_created_ccc['Entity'] = df_created_ccc['Entity_Extra'].map(language_names_full)

# top ccc
df_created_top_ccc = df_created.loc[df_created['set2'] == "top_ccc_articles_lists"]
df_created_top_ccc = df_created_top_ccc.rename(columns={'set2descriptor':'Entity','set2':'Content_Type'})
df_created_top_ccc['Entity'] = df_created_top_ccc['Entity'].map(list_dict_inv)
df_created_top_ccc['Entity_Extra'] = ''

# gender
df_created_gender = df_created.loc[(df_created['set2descriptor'].isin(['male','female']) & (df_created['set2']!="top_ccc_articles_lists"))]
df_created_gender = df_created_gender.rename(columns={'set2descriptor':'Entity'})
df_created_gender['Entity'] = df_created_gender['Entity'].map({'male':'Men','female':'Women'})
df_created_gender['Entity_Extra'] = ''
df_created_gender['Content_Type'] = 'gender'

# geographical entities
df_created_geo = df_created.loc[df_created['Group']=='wp']
df_created_country = df_created_geo.loc[df_created_geo['set2']=='countries']
df_created_country['Entity'] = df_created_country['set2descriptor'].map(country_names)
df_created_country['Entity_Extra'] = df_created_country['set2descriptor'].map(regions)
df_created_country = df_created_country.rename(columns={'set2':'Content_Type'})

df_created_subregion = df_created_geo.loc[df_created_geo['set2']=='subregions']
df_created_subregion = df_created_subregion.rename(columns={'set2descriptor':'Entity','set2':'Content_Type'})
df_created_subregion['Entity_Extra'] = df_created_subregion['Entity'].map(subregions_regions)

df_created_regions = df_created_geo.loc[df_created_geo['set2']=='regions']
df_created_regions = df_created_regions.rename(columns={'set2descriptor':'Entity','set2':'Content_Type'})
df_created_regions['Entity_Extra'] = ''

# print ('created')
# print (df_created_ccc.head(10))
# print (df_created_top_ccc.head(10))
# print (df_created_gender.head(10))
# print (df_created_country.head(10))
# print (df_created_subregion.head(10))
# print (df_created_regions.head(10))
#print(str(datetime.timedelta(seconds=time.time() - functionstartTime)))



# ACCUMULATED DATA ################################################################################
query = 'select set1, set1descriptor, set2, set2descriptor, abs_value, rel_value, period from wcdo_intersections_accumulated where content="articles" AND (set2 IN ("countries","subregions","regions","top_ccc_articles_lists","wikidata_article_qitems") or set2descriptor IN ("ccc","geolocated")) order by set1, period;'
df_accumulated = pd.read_sql_query(query, conn).round(1)

#df_accumulated.to_csv('moc.csv')
# df_accumulated = pd.read_csv('moc.csv')

df_accumulated['period_string'] = df_accumulated['period']; df_accumulated['period'] = pd.to_datetime(df_accumulated['period'])

df_accumulated = df_accumulated.rename(columns={'set1':'Wiki','set1descriptor':'Group','abs_value':'Articles','rel_value':'Extent Articles (%)'})
df_accumulated['Language'] = df_accumulated['Wiki'].map(language_names_full)
df_accumulated['Region']=df_accumulated['Wiki'].map(languages.region)

# wp
df_accumulated_wp = df_accumulated.loc[(df_accumulated['set2descriptor'].isin(["ccc"])) & (df_accumulated['Group']=='wp') & (df_accumulated['Wiki']==df_accumulated['set2'])] #
df_accumulated_wp = df_accumulated_wp.rename(columns={'set2descriptor':'Content_Type'})
df_accumulated_wp['Articles'] = round(df_accumulated_wp['Articles']/(df_accumulated_wp['Extent Articles (%)']/100))
df_accumulated_wp['Extent Articles (%)'] = '100.0'
df_accumulated_wp['Content_Type'] = 'wp'
row_to_append = []
expected = 0
old_art = 0
for i, row in df_accumulated_wp.iterrows():
    cur_period = row['period']
    cur_month = cur_period.month
    cur_year = cur_period.year
    cur_lang = row['Wiki']
    cur_art = row['Articles']
    cur_extent = row['Extent Articles (%)']

    if cur_month > 9: s = str(cur_month)
    else: s = '0'+str(cur_month)
    cur = str(cur_year) + '-' + s

    if ((cur != expected and cur_month!=1 and expected != 0) or (cur_art < old_art and old_art != 0)) and cur_lang == old_lang:
        if cur_art < old_art: 
            df_accumulated_wp.at[i, 'Articles'] = old_art
        try:
            row_to_append.append( pd.Series(data = {'Wiki':cur_lang, 'Group':'wp', 'Content_Type':'wp', 'set2':cur_lang, 'Articles':old_art, 'Extent Articles (%)':old_extent, 'period':pd.to_datetime(expected), 'Language':language_names_full[cur_lang]}))
        except:
            pass
    if cur_month == 12: expected = str(cur_year) + '-01'
    else:
        if int(cur_month) > 8: s = str(int(cur_month)+1)
        else: s = '0'+str(cur_month+1)
        expected = str(cur_year) + '-' + s

    old_lang = cur_lang
    old_art = cur_art
    old_extent = cur_extent

for r in row_to_append:
    df_accumulated_wp = df_accumulated_wp.append(r, ignore_index=True)
df_accumulated_wp = df_accumulated_wp.sort_values(by=['Wiki','period'])
df_accumulated_wp['Entity'] = 'Entire Wikipedia'


# language CCC
df_accumulated_ccc = df_accumulated.loc[(df_accumulated['set2descriptor'] == "ccc") & (df_accumulated['Group']=='wp')]
df_accumulated_ccc = df_accumulated_ccc.rename(columns={'set2':'Entity_Extra','set2descriptor':'Content_Type'})
df_accumulated_ccc['Entity'] = df_accumulated_ccc['Entity_Extra'].map(language_names_full)

# top ccc
df_accumulated_top_ccc = df_accumulated.loc[df_accumulated['set2'] == "top_ccc_articles_lists"]
df_accumulated_top_ccc = df_accumulated_top_ccc.rename(columns={'set2descriptor':'Entity','set2':'Content_Type'})
df_accumulated_top_ccc['Entity'] = df_accumulated_top_ccc['Entity'].map(list_dict_inv)
df_accumulated_top_ccc['Entity_Extra'] = ''

# gender
df_accumulated_gender = df_accumulated.loc[(df_accumulated['set2descriptor'].isin(['male','female'])) & (df_accumulated['set2']=="wikidata_article_qitems")]
df_accumulated_gender = df_accumulated_gender.rename(columns={'set2descriptor':'Entity'})
df_accumulated_gender['Entity'] = df_accumulated_gender['Entity'].map({'male':'Men','female':'Women'})
df_accumulated_gender['Entity_Extra'] = ''
df_accumulated_gender['Content_Type'] = 'gender'

# geographical entities
df_accumulated_gl = df_accumulated.loc[(df_accumulated['set2descriptor']=='geolocated')&(df_accumulated['Group']=='wp')&(df_accumulated['set2']=='languagecode')]
df_accumulated_gl['Wiki:period'] = df_accumulated_gl['Wiki']+':'+df_accumulated_gl['period_string']
df_accumulated_gl = df_accumulated_gl.set_index('Wiki:period')

df_accumulated_geo = df_accumulated.loc[df_accumulated['Group']=='geolocated']
df_accumulated_geo['Wiki:period'] = df_accumulated_geo['Wiki']+':'+df_accumulated_geo['period_string']
df_accumulated_geo = df_accumulated_geo.set_index('Wiki:period')

df_accumulated_geo['WP Articles'] = 100*(df_accumulated_gl['Articles']/df_accumulated_gl['Extent Articles (%)'])
df_accumulated_geo['Extent Articles GL (%)'] = df_accumulated_geo['Extent Articles (%)']; 
df_accumulated_geo['Group'] = 'wp'
df_accumulated_geo['Extent Articles (%)'] = 100*(df_accumulated_geo['Articles']/df_accumulated_geo['WP Articles'])
df_accumulated_geo = df_accumulated_geo.reset_index().round(1)

df_accumulated_country = df_accumulated_geo.loc[df_accumulated_geo['set2']=='countries']
df_accumulated_country['Entity'] = df_accumulated_country['set2descriptor'].map(country_names)
df_accumulated_country['Entity_Extra'] = df_accumulated_country['set2descriptor'].map(regions)
df_accumulated_country = df_accumulated_country.rename(columns={'set2':'Content_Type'})

df_accumulated_subregion = df_accumulated_geo.loc[df_accumulated_geo['set2']=='subregions']
df_accumulated_subregion = df_accumulated_subregion.rename(columns={'set2descriptor':'Entity','set2':'Content_Type'})
df_accumulated_subregion['Entity_Extra'] = df_accumulated_subregion['Entity'].map(subregions_regions)

df_accumulated_regions = df_accumulated_geo.loc[df_accumulated_geo['set2']=='regions']
df_accumulated_regions = df_accumulated_regions.rename(columns={'set2descriptor':'Entity','set2':'Content_Type'})
df_accumulated_regions['Entity_Extra'] = ''

# print ('accumulated')
# print (df_accumulated_ccc.head(10))
# print (df_accumulated_top_ccc.head(10))
# print (df_accumulated_gender.head(10))
# print (df_accumulated_country.head(10))
# print (df_accumulated_subregion.head(10))
# print (df_accumulated_regions.head(10))
#print(str(datetime.timedelta(seconds=time.time() - functionstartTime)))
...
def entity_to_dfs(content_type, created_accumulated):
    ...
    if content_type == 'Languages CCC':
        if created_accumulated == 'created':
           return df_created_ccc
        else:
           return df_accumulated_ccc

    if content_type == 'Top CCC Lists':
        if created_accumulated == 'created':
           return df_created_top_ccc
        else:
           return df_accumulated_top_ccc

    if content_type == 'Gender':
        if created_accumulated == 'created':
           return df_created_gender
        else:
           return df_accumulated_gender

    if content_type == 'Countries':
        if created_accumulated == 'created':
           return df_created_country
        else:
           return df_accumulated_country

    if content_type == 'Subregions':
        if created_accumulated == 'created':
           return df_created_subregion
        else:
           return df_accumulated_subregion

    if content_type == 'Regions':
        if created_accumulated == 'created':
           return df_created_regions
        else:
           return df_accumulated_regions

    if content_type == 'Entire Wikipedia':
        if created_accumulated == 'created':
           return df_created_wp
        else:
           return df_accumulated_wp
...
#    if language == None or content_type == None or period == None or othercontent == None: return None
    ...
    df_created = entity_to_dfs(content_type,'created')
    df_accumulated = entity_to_dfs(content_type,'accumulated')
    ...
    if language != None and period != None and content_type != None: 
        ...
        if othercontent == 'Yes':
            ...
            row_df_created = pd.Series(data = {'Wiki':language, 'Group':'wp', 'Content_Type':content_type, 'Entity':'Other Content', 'Articles':remainingart, 'Extent Articles (%)':remainingpercent, 'period':max_period_monthly, 'Language':language_names_full[language]})
...
def create_fig_barchart(created_accumulated, df, time_aggregation, content_type, absolute_relative):
    ...
    if time_aggregation != '':
        ...
        for entity_name in list(df.Entity.unique()):
...
def update_monthly_barchart(language, content_type, time_aggregation, absolute_relative):
    ...
    if content_type != '':
        # CREATED
        df_created = df_created.loc[(df_created['Wiki'] == language_names[language])]
        ...
        # ACCUMULATED
        df_accumulated = df_accumulated.loc[(df_accumulated['Wiki'] == language_names[language])]
    ...
    fig = create_fig_barchart('Accumulated Articles', df_accumulated, time_aggregation, content_type, absolute_relative)
    fig2 = create_fig_barchart('Created Articles', df_created, time_aggregation, content_type, absolute_relative)
