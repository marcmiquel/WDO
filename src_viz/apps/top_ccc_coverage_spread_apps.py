...
conn2 = sqlite3.connect(databases_path + 'top_ccc_articles.db'); cursor2 = conn2.cursor()
...
#        query = 'SELECT set1descriptor, set1, rel_value, abs_value FROM wcdo_intersections WHERE set2 = "'+language_covering+'" AND set2descriptor = "wp" AND set1 NOT LIKE "%(%" ORDER BY set1, set1descriptor DESC;'
        a = lists_p.copy()
        a.append("%(%")
        page_asstring = ','.join( ['?'] * (len(a)-1) )
        query = 'SELECT set1descriptor, set1, rel_value, abs_value FROM wcdo_intersections WHERE set2 = "'+language_covering+'" AND set2descriptor = "wp" AND measurement_date IN (SELECT MAX(measurement_date) FROM wcdo_intersections) AND set1descriptor IN (%s) ' % page_asstring
        query += 'AND set1 NOT LIKE ? ORDER BY set1, set1descriptor DESC;' 

#        query = 'SELECT set1descriptor, set1, rel_value, abs_value FROM wcdo_intersections WHERE set2 = "'+language_covering+'" AND set2descriptor = "wp" AND measurement_date IN (SELECT MAX(measurement_date) FROM wcdo_intersections) AND set1 NOT LIKE "%(%" ORDER BY set1, set1descriptor DESC;'
...
#                print (languagecode_covered,value)
    ...
    #            print (row_dict)
        ...
        # df = df.sort_values('languagename',ascending=True)
...
#        dash_app8.title = title+title_addenda
        ...
        text = '''This page shows some statistics that explain how well '''+language_cover+''' Wikipedia language edition covers the Top 100 of the [Top CCC articles lists](https://wcdo.wmflabs.org/top_ccc_articles/) from other Wikipedia language editions. Every cell of the table is a clickable link to a Top CCC list.'''
        ...
        title = 'Languages Top CCC articles lists coverage'
#        dash_app8.title = title+title_addenda

        text = '''This page shows some statistics that explain how well a Wikipedia language edition covers the Top 100 of the [Top CCC articles lists](https://wcdo.wmflabs.org/top_ccc_articles/) from other Wikipedia language editions. Every cell of the table is a clickable link to a Top CCC list.
...
#        dash_app9.title = title+title_addenda
        text = '''
                This page shows some statistics that explain how well '''+language_cover+''' Wikipedia language edition covers the [Top CCC articles lists](https://wcdo.wmflabs.org/top_ccc_articles/) generated for the countries associated to each language edition (when it is a region or several regions located in the same country, it appears as one row with the country name). Every cell of the table is a clickable link to a Top CCC list.

                These lists are created by ranking the articles according to specific features and sometimes giving them weights. These different features are based on the _**article characteristics**_ (e.g. number of Editors), _**part of CCC**_ (e.g. geolocated articles), topics such as _**wiki loves**_ (e.g. monuments) or other _**diversity**_ (e.g. women). 
...
conn2 = sqlite3.connect(databases_path + 'stats.db'); cursor2 = conn2.cursor()
...
#        dash_app10.title = title+title_addenda
        ...
        text = '''
                This page shows some statistics that explain how well the first '''+language_spread+''' Wikipedia Top 100 CCC articles are spread across the other language editions. Every cell of the table is a clickable link to a Top CCC list.
...
#        dash_app10.title = title+title_addenda
        ...
        text = '''
                This page shows some statistics that explain how well the first Wikipedia Top 100 CCC articles are spread across the other language editions. 
